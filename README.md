...
`soul.md` 用来保存 Momus 的人格设定、讽刺边界、语气规则和确认问题风格。`SKILL.md` 仍然是触发策略和执行流程的主文件；当两者有重叠时，以 `SKILL.md` 的操作规则为准，以 `soul.md` 补充表达方式。
...
以下用例来自 Momus Skill 的典型触发场景，可用于验证 skill 是否按预期识别弱 prompt、给出有效 critique，并重写成可执行的 IDE-agent prompt。
...
- 应触发 Momus，因为请求缺少错误信息、复现步骤、相关接口、期望行为和验证方式。
- 应要求 agent 先阅读仓库并定位登录接口相关代码。
- 应把修复范围限制在登录流程相关文件内。
- 应要求补充或运行可验证该错误的测试。

![Software bug prompt review](assets/use-cases/software-bug-review.png)
...
- 应触发 Momus，因为“好看”没有定义产品类型、目标用户、页面结构、视觉方向、交互状态或响应式验收标准。
- 应要求 agent 检查现有技术栈、样式系统和组件约定。
- 应把输出目标改写为真实可用页面，而不是通用营销模板。
- 应要求桌面和移动视口截图验证无重叠、无溢出。

![Frontend page prompt review](assets/use-cases/frontend-page-review.png)
...
- 应触发 Momus，因为请求缺少目标平台、环境变量、构建命令、测试门槛、回滚方式和生产验证。
- 应要求 agent 先检查部署配置、构建脚本和 README 运行说明。
- 应明确不泄露密钥、不执行破坏性发布操作。
- 应要求构建、测试和生产 URL 健康检查。

![Deployment prompt review](assets/use-cases/deployment-review.png)
...
本项目使用 MIT License 开源。你可以自由使用、复制、修改、合并、发布、分发、再授权或销售本项目的副本，但需要保留原始版权声明和许可证声明。
